...
function onChangeSelectMsg(checked: CheckboxValueType, msgItem: InteractiveMessageType) {
  if (settings.value.select.value.type === 'forward') {
    ...
    if (checked && settings.value.select.value.selectedList.length >= 10) {
      ElMessage.warning(t('messageLimitWarning'));
      return;
    }
  }
...
<i18n>
{
  "en": {
    ...
    "exitMember": "Exited Member",
    "messageLimitWarning": "You can select a maximum of 10 messages"
  },
  "ja": {
    "forwardMessage": "転送メッセージ",
    "tagMember": "メンバーをタグ付け",
    "viewContactCard": "名刺を見る",
    "exitMember": "退出済みメンバー",
    "messageLimitWarning": "最大で10のメッセージを選択できます"
  }
}
</i18n>
